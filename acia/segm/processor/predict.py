...
def prepare_contours(
    bboxes, segm_result, labels, offset_x=0, offset_y=0, seg_score_threshold=0.3
):
...
def generate_video(input_folder, output_file):
    ...
    (
        ffmpeg.input("output/*.png", pattern_type="glob", framerate=1)
        .output("output.mp4")
        .run()
    )
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Enables tiling image into several smaller images. Especially helpful when many cells are present",
    )
    parser.add_argument(
        "--padding",
        dest="padding",
        type=int,
        default=0,
        help="Enables pixel padding of the images",
    )
    parser.add_argument(
        "--out-file", type=str, default="output.tif", help="Output file"
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="Draw a bounding box around object detections",
    )
    ...
    config_file = (
        args.config
    )  # 'mass_evaluation/me_cascade_mask_rcnn_2_x101_64x4d_False_True_15_True/me_cascade_mask_rcnn_2_x101_64x4d_False_True_15_True.py'#'htc_best.py'#'mass_evaluation/me_htc_2_x101_64x4d_True_True_15_True/me_htc_2_x101_64x4d_True_True_15_True.py'#'mask_rcnn_r50_fpn_1x_coco.py'
    input_tif = (
        args.input
    )  # 'input/PHH2.nd2-PHH2.nd2(series6)_rois-1_70_final.tif' #'input/PHH1001.nd2-PHH1001.nd2(series11)_preprocessed_cropped_rois-1_48_final-1.tif' #'input/2021-01-20 E.coli SIMBAL GFP_LB_CGM9_30°C_bat_pos37.tif_primed.tif'#'input/nd059.nd2 - nd059.nd2 (series 81)-80_crop.tif'
    out_file = args.out_file
    path = (
        args.weights
    )  # 'mass_evaluation/me_cascade_mask_rcnn_2_x101_64x4d_False_False_15_False/epoch_15.pth'#'work_dirs/htc_best/latest.pth'#'mass_evaluation/me_htc_2_x101_64x4d_True_True_15_True/latest.pth'#args.weights
    mode = args.mode
    device = "cuda:0"
    ...
    model = init_detector(
        ...
        cfg_options={
            ...
            "test_pipeline[1].scale_factor": 1.0,
    ...
    for i, image in tqdm.tqdm(enumerate(images)):
        ...
        if mode == "mask":
            ...
            overlay = (
                joint_mask[:, :, None]
                * np.array((0, 255, 0), dtype=np.uint8)[None, None]
            )
        ...
        if mode not in ["tif", "roi"]:
            cont_image = np.where(
                overlay > 0, overlay, image
            )  # cv2.addWeighted(image, 1.0, overlay, 0.6, 0.)
            print("Write image")
            cv2.imwrite("output/%04d.png" % i, cont_image)

    print(
        "Raw prediction duration %.2fs or %.2fs/images"
        % (duration, duration / len(images))
    )

    if mode == "tif":
        filename = (
            args.out_file if not (args.out_file is None) else "output.tif"
        )  # 'output.tif'
