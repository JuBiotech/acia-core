...
if __name__ == "__main__":
    ...
    # credentials
    credentials = dict(
        username="root", host="ibt056", port=4064, secure=True, passwd="omero"
    )

    credentials2 = dict(serverUrl="ibt056", username="root", password="omero")
    ...
    for filename in glob.glob(os.path.join(simulated_data_path, "*.tif")):
        ...
        # upload RoIs
        print(
            f"Filename: {filename}, imageId: {image_id}, overlay size: {len(ijrs.overlay.contours)}"
        )
