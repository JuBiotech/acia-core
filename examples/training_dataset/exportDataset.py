...
def has_all_tags(object, tag_list: List[str] = [], case_sensitive = False)-> bool:
    """[summary]
...
def irs_from_image_list(image_list, conn):
    ...
    for image_id in image_list:
        ...
        if len(gold_range) == 0:
            logging.warning(f'{image.getName()}: No gold-standard range defined!')
            #continue
        irs = ImageRoISource(
    ...
    with BlitzGateway(username, password, host=serverUrl, port=4064, secure=True) as conn:
        ...
        if selected_projects is None:
            ...
            for project_id in selected_projects:
                for dataset in list_datasets_in_project(conn=conn, projectId=project_id):
                    dataset_list.append(dataset.getId())
            ...
            for image in conn.getObjects("Image", opts={'dataset': dataset_id}):
                if all_gold_standard or has_all_tags(image, ['gold-standard']):
                    ...
                    if all_train or has_all_tags(image, ['train']):
                        ...
                        logging.warning("No dataset (train | val) association: put into train")
                        image_train_list.append(image.getId())
